...
def index(request):
    ...
    #
    context = {'scores' : listOfScores, 'workouts' : listOfWorkouts, 'divisions' : listOfDivisions, 'scoringStyle' : scoringStyle.scoringStyle, 'currentDivision' : division, 'currentWorkout' : workout}
...
def scoreInputReceived(request):
    ...
    for (i, teamScore) in enumerate(listOfTeams):
        ...
        try:
            score.save()
            listOfSaveResults.append('Updated score for team ' + listOfTeams[i] + ' to ' + str(score.minutes) + ' minutes, ' + str(score.seconds) + ' seconds, ' + str(score.reps) + ' reps and ' +  str(score.weight) + ' weight')
        except:
            listOfSaveResults.append('unable to save score for team ' + listOfTeams[i])
